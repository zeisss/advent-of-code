...
fn is_valid_passphrase(s: &str) -> bool {
	let mut words : Vec<_> = s.split_whitespace()
		.map(|word| {
			let mut w : Vec<_> = String::from(word).into_bytes();
			w.sort();
...
fn count_valid_passphrases(s: &str) -> (usize, usize) {
	let lines : Vec<_> = s.lines().collect();

	let valid : usize = lines.iter().filter(|passphrase| is_valid_passphrase(passphrase))
		.count();
...
fn main() {
	let input = "vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh
...
xzvfbf fopmfxu mvftgr mfupoxf coyhof talcc vpkslo";
	...
	let (valid, total) = count_valid_passphrases(input);
    println!("{} of {} are valid", valid, total);
}
